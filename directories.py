...
def train(sdir):
    ...
    for i in sdir:
        ...
        for k in range(0,len(to_train)-1):
            sfile = to_train[k]
            # print ('COPYING: ' + PROJECT_HOME + '/' + cdir + '/' + sfile + 'TO: ' + PROJECT_HOME + '/train/' + i + '_train/' + sfile)
            shutil.move(PROJECT_HOME + '/' + cdir + '/' + sfile, PROJECT_HOME + '/train/' + i + '_train/' + sfile)
...
def test(sdir):
    for i in sdir:
        ...
        rem = os.listdir(cdir)
        for k in rem:
            shutil.move(PROJECT_HOME + '/' + cdir + '/' + k, PROJECT_HOME + '/test/' + i + '_test/' + k)
...
def valid(sdir):
    ...
    for i in sdir:
        ...
        for k in range(0, len(to_valid)-1):
            sfile = to_valid[k]
            shutil.move(PROJECT_HOME + '/' + cdir + '/' + sfile, PROJECT_HOME + '/valid/' + i + '_valid/' + sfile)
